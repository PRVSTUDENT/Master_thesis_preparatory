...
C ======================================================================
C SIGINI: Initialize residual stress from extracted cycle-99 data.
C ======================================================================
...
C     Predicted cycle-99 stress prepared from cycle-10 state (Stage 10B)
C     Source: stage5_predicted_cycle_jump/cycle99_predicted_stress_for_injection.csv

      IF (NTENS.GE.1) SIGMA(1) = 364.758456760307D0
      IF (NTENS.GE.2) SIGMA(2) = -1.08949886150262D-13
      IF (NTENS.GE.3) SIGMA(3) = -1.17732983855789D-13
      IF (NTENS.GE.4) SIGMA(4) = 3.5128802054301D-13
      IF (NTENS.GE.5) SIGMA(5) = -1.25658494235178D-13
      IF (NTENS.GE.6) SIGMA(6) = 4.30920065761671D-14

      RETURN
      END

C ======================================================================
C SDVINI: Initialize state-dependent variables from extracted cycle-99
...
C     Predicted cycle-99 state prepared from cycle-10 state (Stage 10B)
C     Source: stage5_predicted_cycle_jump/cycle99_predicted_statev_for_injection.csv

      STATEV(1 ) = 0.70977318519734D0       ! STATEV1: accumulated viscoplastic strain
      STATEV(2 ) = -69.1144027709321D0       ! STATEV2: backstress X1
      STATEV(3 ) = 34.5572013854759D0       ! STATEV3: backstress X2
      STATEV(4 ) = 34.5572013854759D0       ! STATEV4: backstress X3
      STATEV(5 ) = 1.84106295429261D-14       ! STATEV5: near-zero shear component
      STATEV(6 ) = 3.86662374646242D-15       ! STATEV6: near-zero shear component
      STATEV(7 ) = 4.09923686839554D-17       ! STATEV7: near-zero shear component
      STATEV(8 ) = -0.00193178987441205D0       ! STATEV8: viscoplastic strain Ep1
      STATEV(9 ) = 0.000965894937205033D0       ! STATEV9: viscoplastic strain Ep2
      STATEV(10) = 0.000965894937205033D0       ! STATEV10: viscoplastic strain Ep3
      STATEV(11) = -3.10259648751638D-18       ! STATEV11: near-zero shear component
      STATEV(12) = -1.82841601841459D-18       ! STATEV12: near-zero shear component
      STATEV(13) = -1.44896828290513D-19       ! STATEV13: near-zero shear component
      STATEV(14) = 6.97326410036212D0       ! STATEV14: recomputed isotropic hardening
