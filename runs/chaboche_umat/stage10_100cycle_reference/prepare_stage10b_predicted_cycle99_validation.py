...
THIS_DIR = os.path.join(ROOT, "stage10_100cycle_reference", "target99_cycle100")
...
HISTORY_CSV = os.path.join(
    ROOT,
    "stage10_100cycle_reference",
    "reference_100cycles",
    "chaboche_vp_v1_cyclic_eps005_100cycles_cycle_history.csv"
)
...
JOB = "chaboche_stage10b_predicted_cycle99_to_cycle100"
CHECK_JOB = JOB + "_check"
TARGET_UMAT = os.path.join(THIS_DIR, "umat_chaboche_v1_with_sdvini_sigini_predicted_cycle99.f")
TARGET_DECK = os.path.join(THIS_DIR, JOB + ".inp")
RUN_BAT = os.path.join(THIS_DIR, "run_stage10b_predicted_cycle99_jump.bat")
POSTPROCESS_PY = os.path.join(THIS_DIR, "postprocess_stage10b_predicted_cycle99_jump.py")
MONITOR_PY = os.path.join(THIS_DIR, "monitor_stage10b_predicted_cycle99_jump.py")

PRED_STATEV_CSV = os.path.join(THIS_DIR, "cycle99_predicted_statev_for_injection.csv")
PRED_STRESS_CSV = os.path.join(THIS_DIR, "cycle99_predicted_stress_for_injection.csv")
ERROR_CSV = os.path.join(THIS_DIR, "cycle99_predicted_vs_exact_error.csv")
REFERENCE_CSV = os.path.join(THIS_DIR, "cycle100_reference_statev_stress.csv")
RESULT_CSV = os.path.join(THIS_DIR, "stage10b_predicted_cycle99_jump_result.csv")
PREP_REPORT = os.path.join(THIS_DIR, "STAGE10B_PREDICTED_CYCLE99_STATE_PREP_REPORT.md")
RESULT_REPORT = os.path.join(THIS_DIR, "STAGE10B_PREDICTED_CYCLE99_JUMP_RESULT_REPORT.md")
...
def write_prediction_csvs(base, exact, reference, pred_statev, pred_stress, statev_mean, stress_mean):
    ...
    with csv_open_write(ERROR_CSV) as handle:
        fields = ["quantity", "predicted_cycle99", "exact_cycle99", "absolute_error", "relative_error", "relative_error_percent", "policy"]
        writer = csv.DictWriter(handle, fieldnames=fields)
        writer.writeheader()
        for i in range(1, NSTATEV + 1):
            ...
            writer.writerow({
                "quantity": "STATEV%d" % i,
                "predicted_cycle99": fmt(pred_statev[i]),
                "exact_cycle99": fmt(exact_value),
        ...
        for name in STRESS_COMPONENTS:
            ...
            writer.writerow({
                "quantity": name,
                "predicted_cycle99": fmt(pred_stress[name]),
                "exact_cycle99": fmt(exact_value),
    ...
    with csv_open_write(REFERENCE_CSV) as handle:
        fields = ["quantity", "cycle100_history_row_value"]
        writer = csv.DictWriter(handle, fieldnames=fields)
        writer.writeheader()
        for i in range(1, NSTATEV + 1):
            writer.writerow({"quantity": "STATEV%d" % i, "cycle100_history_row_value": fmt(reference["STATEV%d_end" % i])})
        for name in STRESS_COMPONENTS + ["RIGHT_FACE_U1_AVG", "RIGHT_FACE_RF1_SUM"]:
            writer.writerow({"quantity": name, "cycle100_history_row_value": fmt(reference.get(name))})
...
def write_umat(pred_statev, pred_stress):
    ...
    text = text.replace("cycle-29", "cycle-99")
    text = text.replace("cycle 29", "cycle 99")
    text = text.replace("cycle29", "cycle99")
    text = text.replace("Stage 6D", "Stage 10B")
    text = text.replace("Stage 5A", "Stage 10B")
    text = text.replace("cycle19_predicted", "cycle99_predicted")
...
def write_deck():
    ...
    replacements = {
        "Stage 6D": "Stage 10B",
        "Predicted cycle-29 to cycle-30 FE cycle-jump validation": "Predicted cycle-99 to cycle-100 FE cycle-jump validation",
        "Predicted Cycle-29 to Cycle-30 FE Cycle Jump": "Predicted Cycle-99 to Cycle-100 FE Cycle Jump",
        "selected from the Stage 6C scan": "selected by the Stage 10B grouped adaptive controller",
        "cycle-29": "cycle-99",
        "cycle-30": "cycle-100",
        "cycle 30": "cycle 100",
        "cycle 29": "cycle 99",
...
def write_prep_report(pred_statev, pred_stress, exact, reference):
    ...
    lines = [
        ...
        "- Exact cycle-99 STATEV1: `%s`" % fmt(exact["STATEV1_end"]),
        "- STATEV1 relative error: `%s%%`" % fmt(statev1_rel),
        "- Predicted S11: `%s MPa`" % fmt(pred_stress["S11"]),
        "- Exact cycle-99 S11: `%s MPa`" % fmt(exact["S11"]),
        ...
        "- Cycle-100 reference CSV: `%s`" % os.path.basename(REFERENCE_CSV),
