...
C ======================================================================
C SIGINI: Initialize residual stress from extracted cycle-499 data.
C ======================================================================
...
C     Predicted cycle-499 stress prepared from cycle-10 state (Stage 11B)
C     Source: stage5_predicted_cycle_jump/cycle499_predicted_stress_for_injection.csv

      IF (NTENS.GE.1) SIGMA(1) = 493.480244954947D0
      IF (NTENS.GE.2) SIGMA(2) = -5.82645043325373D-13
      IF (NTENS.GE.3) SIGMA(3) = -6.30902737460233D-13
      IF (NTENS.GE.4) SIGMA(4) = 1.91405959600701D-12
      IF (NTENS.GE.5) SIGMA(5) = -6.93524557492067D-13
      IF (NTENS.GE.6) SIGMA(6) = 2.06264904403345D-13

      RETURN
      END

C ======================================================================
C SDVINI: Initialize state-dependent variables from extracted cycle-499
...
C     Predicted cycle-499 state prepared from cycle-10 state (Stage 11B)
C     Source: stage5_predicted_cycle_jump/cycle499_predicted_statev_for_injection.csv

      STATEV(1 ) = 3.58395926142134D0       ! STATEV1: accumulated viscoplastic strain
      STATEV(2 ) = 6.51731109633948D0       ! STATEV2: backstress X1
      STATEV(3 ) = -3.25865554811568D0       ! STATEV3: backstress X2
      STATEV(4 ) = -3.25865554811568D0       ! STATEV4: backstress X3
      STATEV(5 ) = 9.26560024084816D-14       ! STATEV5: near-zero shear component
      STATEV(6 ) = 1.41817692142935D-14       ! STATEV6: near-zero shear component
      STATEV(7 ) = 8.22536190070173D-15       ! STATEV7: near-zero shear component
      STATEV(8 ) = -0.0025447553489381D0       ! STATEV8: viscoplastic strain Ep1
      STATEV(9 ) = 0.0012723776744681D0       ! STATEV9: viscoplastic strain Ep2
      STATEV(10) = 0.0012723776744681D0       ! STATEV10: viscoplastic strain Ep3
      STATEV(11) = -1.59226828309653D-17       ! STATEV11: near-zero shear component
      STATEV(12) = -9.35271754977903D-18       ! STATEV12: near-zero shear component
      STATEV(13) = -5.7385595468518D-19       ! STATEV13: near-zero shear component
      STATEV(14) = 32.8119204624785D0       ! STATEV14: recomputed isotropic hardening
