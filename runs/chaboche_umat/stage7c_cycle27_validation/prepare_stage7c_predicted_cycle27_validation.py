...
JOB = "chaboche_stage7c_predicted_cycle27_to_cycle28"
CHECK_JOB = JOB + "_check"
TARGET_UMAT = os.path.join(THIS_DIR, "umat_chaboche_v1_with_sdvini_sigini_predicted_cycle27.f")
TARGET_DECK = os.path.join(THIS_DIR, JOB + ".inp")
RUN_BAT = os.path.join(THIS_DIR, "run_stage7c_predicted_cycle27_jump.bat")
POSTPROCESS_PY = os.path.join(THIS_DIR, "postprocess_stage7c_predicted_cycle27_jump.py")
MONITOR_PY = os.path.join(THIS_DIR, "monitor_stage7c_predicted_cycle27_jump.py")

PRED_STATEV_CSV = os.path.join(THIS_DIR, "cycle27_predicted_statev_for_injection.csv")
PRED_STRESS_CSV = os.path.join(THIS_DIR, "cycle27_predicted_stress_for_injection.csv")
ERROR_CSV = os.path.join(THIS_DIR, "cycle27_predicted_vs_exact_error.csv")
REFERENCE_CSV = os.path.join(THIS_DIR, "cycle28_reference_statev_stress.csv")
RESULT_CSV = os.path.join(THIS_DIR, "stage7c_predicted_cycle27_jump_result.csv")
PREP_REPORT = os.path.join(THIS_DIR, "STAGE7C_PREDICTED_CYCLE27_STATE_PREP_REPORT.md")
RESULT_REPORT = os.path.join(THIS_DIR, "STAGE7C_PREDICTED_CYCLE27_JUMP_RESULT_REPORT.md")
...
def write_prediction_csvs(base, exact, reference, pred_statev, pred_stress, statev_mean, stress_mean):
    ...
    with csv_open_write(ERROR_CSV) as handle:
        fields = ["quantity", "predicted_cycle27", "exact_cycle27", "absolute_error", "relative_error", "relative_error_percent", "policy"]
        writer = csv.DictWriter(handle, fieldnames=fields)
        writer.writeheader()
        for i in range(1, NSTATEV + 1):
            ...
            writer.writerow({
                "quantity": "STATEV%d" % i,
                "predicted_cycle27": fmt(pred_statev[i]),
                "exact_cycle27": fmt(exact_value),
        ...
        for name in STRESS_COMPONENTS:
            ...
            writer.writerow({
                "quantity": name,
                "predicted_cycle27": fmt(pred_stress[name]),
                "exact_cycle27": fmt(exact_value),
    ...
    with csv_open_write(REFERENCE_CSV) as handle:
        fields = ["quantity", "cycle28_history_row_value"]
        writer = csv.DictWriter(handle, fieldnames=fields)
        writer.writeheader()
        for i in range(1, NSTATEV + 1):
            writer.writerow({"quantity": "STATEV%d" % i, "cycle28_history_row_value": fmt(reference["STATEV%d_end" % i])})
        for name in STRESS_COMPONENTS + ["RIGHT_FACE_U1_AVG", "RIGHT_FACE_RF1_SUM"]:
            writer.writerow({"quantity": name, "cycle28_history_row_value": fmt(reference.get(name))})
...
def write_umat(pred_statev, pred_stress):
    ...
    text = text.replace("cycle-29", "cycle-27")
    text = text.replace("cycle 29", "cycle 27")
    text = text.replace("cycle29", "cycle27")
    text = text.replace("Stage 6D", "Stage 7C")
    text = text.replace("Stage 5A", "Stage 7C")
    text = text.replace("cycle19_predicted", "cycle27_predicted")
...
def write_deck():
    ...
    replacements = {
        "Stage 6D": "Stage 7C",
        "Predicted cycle-29 to cycle-30 FE cycle-jump validation": "Predicted cycle-27 to cycle-28 FE cycle-jump validation",
        "Predicted Cycle-29 to Cycle-30 FE Cycle Jump": "Predicted Cycle-27 to Cycle-28 FE Cycle Jump",
        "selected from the Stage 6C scan": "selected by the Stage 7B grouped adaptive controller",
        "cycle-29": "cycle-27",
        "cycle-30": "cycle-28",
        "cycle 30": "cycle 28",
        "cycle 29": "cycle 27",
        "C29_TO_C30": "C27_TO_C28",
    }
...
ODB_PATH = "chaboche_stage7c_predicted_cycle27_to_cycle28.odb"
REFERENCE_ODB_PATH = "../chaboche_vp_v1_cyclic_eps005_50cycles.odb"
REFERENCE_TIME = 28.0
CSV_PATH = "stage7c_predicted_cycle27_jump_result.csv"
REFERENCE_CSV = "cycle28_reference_statev_stress.csv"
REPORT_PATH = "STAGE7C_PREDICTED_CYCLE27_JUMP_RESULT_REPORT.md"
...
def write_reference_csv(reference):
    with csv_open_write(REFERENCE_CSV) as handle:
        writer = csv.DictWriter(handle, fieldnames=["quantity", "cycle28_reference_value", "source", "lower_time", "upper_time", "alpha"])
        writer.writeheader()
        for i in range(1, 16):
            writer.writerow({"quantity": "STATEV%%d" %% i, "cycle28_reference_value": fmt(reference["STATEV%%d" %% i]), "source": reference["source"], "lower_time": fmt(reference["lower_time"]), "upper_time": fmt(reference["upper_time"]), "alpha": fmt(reference["alpha"])})
        for name in STRESS_COMPONENTS + ["RIGHT_FACE_U1_AVG", "RIGHT_FACE_RF1_SUM"]:
            writer.writerow({"quantity": name, "cycle28_reference_value": fmt(reference[name]), "source": reference["source"], "lower_time": fmt(reference["lower_time"]), "upper_time": fmt(reference["upper_time"]), "alpha": fmt(reference["alpha"])})
...
def write_report(rows, reference):
    ...
    lines = [
        ...
        "- Stage 7B DeltaN = 17 accepted: `%%s`" %% ("yes" if outcome.startswith("accepted") else "no"),
        "- Stage 7C outcome: `%%s`" %% outcome,
        "",
        "## Outputs",
        "",
...
def write_prep_report(pred_statev, pred_stress, exact, reference):
    ...
    lines = [
        ...
        "- Exact cycle-27 STATEV1: `%s`" % fmt(exact["STATEV1_end"]),
        "- STATEV1 relative error: `%s%%`" % fmt(statev1_rel),
        "- Predicted S11: `%s MPa`" % fmt(pred_stress["S11"]),
        "- Exact cycle-27 S11: `%s MPa`" % fmt(exact["S11"]),
        ...
        "- Cycle-28 reference CSV: `%s`" % os.path.basename(REFERENCE_CSV),
