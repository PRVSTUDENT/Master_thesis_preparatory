...
def block_dir(stage14b, strategy, block_index, base_cycle, target_cycle, continue_to_cycle):
    return os.path.join(
        stage14b,
...
def create_block(args):
    ...
    stage14b = os.path.join(repo, "runs", "chaboche_umat", "stage14b_adaptive_deltaN_2000cycles")
    ...
    case_dir = block_dir(stage14b, STRATEGY_LABEL, args.block_index, args.base_cycle, target_cycle, continue_to_cycle)
    if not os.path.exists(case_dir):
        os.makedirs(case_dir)

    job = "chaboche_stage14b_adaptive_block%02d_target%d_to%d" % (args.block_index, target_cycle, continue_to_cycle)
    previous_csv = args.previous_route_csv
    route_csv_name = "route_history_until_cycle%d.csv" % continue_to_cycle
    result_csv_name = "stage14b_adaptive_block%02d_result.csv" % args.block_index
    prep_report_name = "STAGE14B_ADAPTIVE_BLOCK%02d_PREP_REPORT.md" % args.block_index
    result_report_name = "STAGE14B_ADAPTIVE_BLOCK%02d_RESULT_REPORT.md" % args.block_index
    ...
    paths = {
        ...
        "runner": os.path.join(case_dir, "run_stage14b_adaptive_block%02d_target%d_to%d.bat" % (args.block_index, target_cycle, continue_to_cycle)),
        ...
        "metadata": os.path.join(case_dir, "stage14b_adaptive_block%02d_metadata.csv" % args.block_index),
    }
