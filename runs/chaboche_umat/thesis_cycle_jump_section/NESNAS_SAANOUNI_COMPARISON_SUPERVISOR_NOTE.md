...
| Jump-size formula | Based on damage evolution rate per cycle | \(\Delta N_i = \left\lfloor \dfrac{\eta A_i}{|\operatorname{mean}(\Delta Y_i)|+\varepsilon} \right\rfloor\) | Same adaptive logic, but applied to Chaboche variables instead of damage. |
