...
# Stage 16N-C fixed state-initialized cycle-jump runner.
# Usage from a prepared case directory:
#   bash ../../run_stage16n_fixed_jump_cases_hpc.sh <job-name>
...
if [[ ! -f "state.csv" ]]; then
  echo "Missing state CSV: state.csv" >&2
  exit 2
fi
if [[ ! -f "state.bin" ]]; then
  echo "Missing state binary: state.bin" >&2
  exit 2
fi
...
echo "Stage 16N-C fixed state-initialized jump job: ${JOB}"
...
echo "Stage 16N-C fixed state-initialized jump completed: ${JOB}"
