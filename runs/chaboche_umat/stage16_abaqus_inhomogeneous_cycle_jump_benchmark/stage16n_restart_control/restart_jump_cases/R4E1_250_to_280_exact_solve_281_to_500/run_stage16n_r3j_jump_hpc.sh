...
echo "[Stage16N-R3J] solved continuation cycles: 281 -> $TARGET_CYCLE"
...
if [[ ! -f state.bin || ! -f "$STATE_SUMMARY" ]]; then
  ...
  if [[ "$STATE_MODE" = "exact_target" ]]; then
    ...
    cp "_jump_state/stage16n_exact_state_cycle$(printf '%04d' "$JUMP_CYCLE").bin" state.bin
    {
      ...
      echo "- Solved continuation cycles: \`281 -> $TARGET_CYCLE\`"
...
CASE_DIR="runs/chaboche_umat/stage16_abaqus_inhomogeneous_cycle_jump_benchmark/stage16n_restart_control/restart_jump_cases/R4E1_250_to_280_exact_solve_281_to_500"
