...
JOB = "chaboche_vp_v1_cyclic_eps005_20cycles_exact"
ODB_PATH = JOB + ".odb"

HISTORY_CSV = "chaboche_v1_full_statev_cycle_history_exact.csv"
STABILITY_CSV = "chaboche_v1_full_statev_cycle_stability_exact.csv"
REPORT = "CHABOCHE_V1_FULL_STATEV_CYCLE_HISTORY_EXACT_REPORT.md"
...
def write_report(history_rows, stability):
    ...
    max_abs_time_error = max(abs(r["time_error"]) for r in history_rows)
    approx_zero = all(abs(r["time_error"]) <= 1.0e-8 for r in history_rows)
    ...
    lines += [
        ...
        "## Exact-Phase Check",
        "",
        "- Maximum absolute cycle-end time_error: `%s`" % fmt(max_abs_time_error),
        "- All time_error values approximately zero: `%s`" % ("yes" if approx_zero else "no"),
        "- Previous nearest-frame max absolute time_error: `%s`" % fmt(PREVIOUS_MAX_TIME_ERROR),
    ]

    if max_abs_time_error <= PREVIOUS_MAX_TIME_ERROR:
        lines.append("")
        lines.append("- Result: max absolute time_error is smaller than or equal to the previous nearest-frame max time error.")
    else:
        lines.append("")
        lines.append("- Result: max absolute time_error is larger than the previous nearest-frame value.")
...
def main():
    ...
    print("Exact-output Full STATEV cycle-history extraction complete")
