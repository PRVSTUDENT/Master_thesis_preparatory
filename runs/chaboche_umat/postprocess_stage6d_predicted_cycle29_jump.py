...
REFERENCE_CSV = "stage6_cycle29_jump/cycle30_reference_statev_stress.csv"

EXPECTED_INITIAL_STATEV1 = 0.20679062185814
EXPECTED_INITIAL_S11 = 342.2321438262454
...
def collect_interpolated_reference():
    ...
    try:
        ...
        if lower is None or upper is None:
            raise RuntimeError("Could not bracket reference time %.12g" % REFERENCE_TIME)
...
def write_reference_csv(reference):
    with csv_open_write(REFERENCE_CSV) as handle:
        writer = csv.DictWriter(handle, fieldnames=[
            "quantity", "cycle30_reference_value", "source", "lower_time", "upper_time", "alpha"
        ])
        writer.writeheader()
        for i in range(1, 16):
            writer.writerow({
                "quantity": "STATEV%d" % i,
                "cycle30_reference_value": fmt(reference["STATEV%d" % i]),
                "source": reference["source"],
                "lower_time": fmt(reference["lower_time"]),
                "upper_time": fmt(reference["upper_time"]),
                "alpha": fmt(reference["alpha"]),
            })
        for name in STRESS_COMPONENTS + ["RIGHT_FACE_U1_AVG", "RIGHT_FACE_RF1_SUM"]:
            writer.writerow({
                "quantity": name,
                "cycle30_reference_value": fmt(reference[name]),
                "source": reference["source"],
                "lower_time": fmt(reference["lower_time"]),
                "upper_time": fmt(reference["upper_time"]),
                "alpha": fmt(reference["alpha"]),
