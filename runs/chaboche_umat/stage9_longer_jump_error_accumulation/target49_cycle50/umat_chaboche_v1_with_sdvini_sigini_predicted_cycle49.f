...
C ======================================================================
C SIGINI: Initialize residual stress from extracted cycle-49 data.
C ======================================================================
...
C     Predicted cycle-49 stress prepared from cycle-10 state (Stage 9B)
C     Source: stage5_predicted_cycle_jump/cycle49_predicted_stress_for_injection.csv

      IF (NTENS.GE.1) SIGMA(1) = 348.668233235977D0
      IF (NTENS.GE.2) SIGMA(2) = -4.97379915033733D-14
      IF (NTENS.GE.3) SIGMA(3) = -5.35867646552333D-14
      IF (NTENS.GE.4) SIGMA(4) = 1.5594157361001D-13
      IF (NTENS.GE.5) SIGMA(5) = -5.46752363280667D-14
      IF (NTENS.GE.6) SIGMA(6) = 2.26953943477698D-14

      RETURN
      END

C ======================================================================
C SDVINI: Initialize state-dependent variables from extracted cycle-49
...
C     Predicted cycle-49 state prepared from cycle-10 state (Stage 9B)
C     Source: stage5_predicted_cycle_jump/cycle49_predicted_statev_for_injection.csv

      STATEV(1 ) = 0.35049992566934D0       ! STATEV1: accumulated viscoplastic strain
      STATEV(2 ) = -78.568367004341D0       ! STATEV2: backstress X1
      STATEV(3 ) = 39.2841835021748D0       ! STATEV3: backstress X2
      STATEV(4 ) = 39.2841835021748D0       ! STATEV4: backstress X3
      STATEV(5 ) = 9.12995793473166D-15       ! STATEV5: near-zero shear component
      STATEV(6 ) = 2.57723056298353D-15       ! STATEV6: near-zero shear component
      STATEV(7 ) = -9.82053822818267D-16       ! STATEV7: near-zero shear component
      STATEV(8 ) = -0.00185516919009629D0       ! STATEV8: viscoplastic strain Ep1
      STATEV(9 ) = 0.000927584595047149D0       ! STATEV9: viscoplastic strain Ep2
      STATEV(10) = 0.000927584595047149D0       ! STATEV10: viscoplastic strain Ep3
      STATEV(11) = -1.50008569458527D-18       ! STATEV11: near-zero shear component
      STATEV(12) = -8.87878326994033D-19       ! STATEV12: near-zero shear component
      STATEV(13) = -9.127693749118D-20       ! STATEV13: near-zero shear component
      STATEV(14) = 3.4744653363548D0       ! STATEV14: recomputed isotropic hardening
