...
C ======================================================================
C SIGINI: Initialize residual stress from extracted cycle-39 data.
C ======================================================================
...
C     Predicted cycle-39 stress prepared from cycle-10 state (Stage 9A)
C     Source: stage5_predicted_cycle_jump/cycle39_predicted_stress_for_injection.csv

      IF (NTENS.GE.1) SIGMA(1) = 345.450188531111D0
      IF (NTENS.GE.2) SIGMA(2) = -3.78956125739956D-14
      IF (NTENS.GE.3) SIGMA(3) = -4.07575208151222D-14
      IF (NTENS.GE.4) SIGMA(4) = 1.1687228422341D-13
      IF (NTENS.GE.5) SIGMA(5) = -4.04785847466444D-14
      IF (NTENS.GE.6) SIGMA(6) = 1.86160719020904D-14

      RETURN
      END

C ======================================================================
C SDVINI: Initialize state-dependent variables from extracted cycle-39
...
C     Predicted cycle-39 state prepared from cycle-10 state (Stage 9A)
C     Source: stage5_predicted_cycle_jump/cycle39_predicted_statev_for_injection.csv

      STATEV(1 ) = 0.27864527376374D0       ! STATEV1: accumulated viscoplastic strain
      STATEV(2 ) = -80.4591598510228D0       ! STATEV2: backstress X1
      STATEV(3 ) = 40.2295799255146D0       ! STATEV3: backstress X2
      STATEV(4 ) = 40.2295799255146D0       ! STATEV4: backstress X3
      STATEV(5 ) = 7.27382361309277D-15       ! STATEV5: near-zero shear component
      STATEV(6 ) = 2.31935192628776D-15       ! STATEV6: near-zero shear component
      STATEV(7 ) = -1.18666306111871D-15       ! STATEV7: near-zero shear component
      STATEV(8 ) = -0.00183984505323314D0       ! STATEV8: viscoplastic strain Ep1
      STATEV(9 ) = 0.000919922526615573D0       ! STATEV9: viscoplastic strain Ep2
      STATEV(10) = 0.000919922526615573D0       ! STATEV10: viscoplastic strain Ep3
      STATEV(11) = -1.17958353599904D-18       ! STATEV11: near-zero shear component
      STATEV(12) = -6.99770788709922D-19       ! STATEV12: near-zero shear component
      STATEV(13) = -8.05529593313133D-20       ! STATEV13: near-zero shear component
      STATEV(14) = 2.7671317728281D0       ! STATEV14: recomputed isotropic hardening
