...
def main():
    ...
    if active_workers != HARD_MAX_WORKERS:
        fail(errors, "STAGE15J_ACTIVE_WORKERS must be %d; got %d" % (HARD_MAX_WORKERS, active_workers))
    elif active_workers < 1:
        fail(errors, "STAGE15J_ACTIVE_WORKERS must be positive")
    else:
        ok("STAGE15J_ACTIVE_WORKERS=%d" % active_workers)
