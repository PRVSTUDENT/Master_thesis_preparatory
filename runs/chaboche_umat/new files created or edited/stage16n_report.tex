...
\begin{tabular}{lllll}
\toprule
Case & PBS job & Restart read & Continuation & Status at first check \\
\midrule
\code{R2C1} & \code{1341284.mmaster02} & step 100, inc 53 & 100 $\rightarrow$ 250 & running on \code{mnode100} \\
\code{R2C2} & \code{1341285.mmaster02} & step 250, inc 58 & 250 $\rightarrow$ 500 & running on \code{mnode101} \\
\bottomrule
\end{tabular}
\end{table}

Both corrected jobs request \code{select=1:ncpus=16:mpiprocs=1:ompthreads=16:mem=90gb}, \code{walltime=24:00:00}, and run Abaqus with \code{cpus=16 mp\_mode=threads}. The first submitted R2 attempt, \code{1341282.mmaster02} and \code{1341283.mmaster02}, failed during Abaqus datacheck because the oldjob \code{.odb} file was not symlinked into the restart case folders. This was a restart-setup issue, not a solver result. The link script was corrected to expose the oldjob \code{.odb} together with \code{.res}, \code{.stt}, \code{.mdl}, \code{.sim}, and \code{.prt}; the corrected submissions then entered running state.

The acceptance check for R2 is comparison of the native restart continuation outputs against the 1000-cycle reference and the R1 restart-enabled references. If R2C2 passes, the cycle-250 state is confirmed to be restartable when Abaqus preserves the finite-element state, strengthening the conclusion that the earlier cycle-250 failure was a limitation of scratch \code{SDVINI}/\code{SIGINI} initialization.
