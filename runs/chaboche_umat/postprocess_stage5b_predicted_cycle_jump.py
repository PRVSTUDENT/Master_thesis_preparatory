...
ODB_PATH = "chaboche_stage5b_predicted_cycle19_to_cycle20.odb"
CSV_PATH = "stage5_predicted_cycle_jump/stage5b_predicted_cycle_jump_result.csv"
REPORT_PATH = "stage5_predicted_cycle_jump/STAGE5B_PREDICTED_CYCLE_JUMP_RESULT_REPORT.md"
EXPECTED_INITIAL_STATEV1 = 0.134935969953
REFERENCE_FINAL_STATEV1 = 0.14202569425106049
EXPECTED_INITIAL_S11 = 339.014099121
REFERENCE_FINAL_S11 = 376.43414306640625
EXACT_SIGINI_FINAL_STATEV1 = 0.141863301396
EXACT_SIGINI_FINAL_S11 = 375.865997314
...
def write_report(rows):
    ...
    lines = [
        ...
        "- Final STATEV1 is within 1%% of explicit cycle-20 reference: `%s`" % ("yes" if abs(final_sdv1 - REFERENCE_FINAL_STATEV1) / abs(REFERENCE_FINAL_STATEV1) < 0.01 else "no"),
        "- Final S11 is within 1%% of explicit cycle-20 reference: `%s`" % ("yes" if abs(final["S11"] - REFERENCE_FINAL_S11) / abs(REFERENCE_FINAL_S11) < 0.01 else "no"),
        ...
        "- Final-frame STATEV1 relative error percent: `%s`" % fmt(100.0 * abs(final_sdv1 - REFERENCE_FINAL_STATEV1) / abs(REFERENCE_FINAL_STATEV1)),
        "- Final-frame S11 relative error percent: `%s`" % fmt(100.0 * abs(final["S11"] - REFERENCE_FINAL_S11) / abs(REFERENCE_FINAL_S11)),
        "- Difference from exact-state SIGINI final STATEV1: `%s`" % fmt(final_sdv1 - EXACT_SIGINI_FINAL_STATEV1),
        "- Difference from exact-state SIGINI final S11: `%s`" % fmt(final["S11"] - EXACT_SIGINI_FINAL_S11),
        "- Successful first FE cycle-skipping demonstration by 1%% criteria: `%s`" % ("yes" if abs(final_sdv1 - REFERENCE_FINAL_STATEV1) / abs(REFERENCE_FINAL_STATEV1) < 0.01 and abs(final["S11"] - REFERENCE_FINAL_S11) / abs(REFERENCE_FINAL_S11) < 0.01 else "no"),
        "",
