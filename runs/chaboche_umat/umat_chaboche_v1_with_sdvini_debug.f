     ...
     4 NOEL,NPT,LAYER,KSPT,JSTEP,KINC)
      ...
      IF (DBG_FIRST) THEN
        OPEN(UNIT=DBGUNIT,FILE='stage4b_sdvini_debug_trace.txt',
     1       STATUS='UNKNOWN',POSITION='APPEND')
        WRITE(DBGUNIT,*) 'UMAT FIRST ENTRY'
        WRITE(DBGUNIT,*) 'NOEL,NPT,LAYER,KSPT=',NOEL,NPT,LAYER,KSPT
        WRITE(DBGUNIT,*) 'KINC=',KINC
        WRITE(DBGUNIT,*) 'JSTEP=',JSTEP(1),JSTEP(2),JSTEP(3),
     1       JSTEP(4)
        WRITE(DBGUNIT,*) 'TIME=',TIME(1),TIME(2),' DTIME=',DTIME
        WRITE(DBGUNIT,*) 'NSTATV=',NSTATV
        WRITE(DBGUNIT,*) 'STATEV1 entering UMAT=',STATEV1_IN
        CLOSE(DBGUNIT)
      END IF
     ...
     1            /(1.D0+GKIN*DP)
      ...
      IF (DBG_FIRST) THEN
        OPEN(UNIT=DBGUNIT,FILE='stage4b_sdvini_debug_trace.txt',
     1       STATUS='UNKNOWN',POSITION='APPEND')
        WRITE(DBGUNIT,*) 'UMAT FIRST EXIT'
        WRITE(DBGUNIT,*) 'STATEV1 entering UMAT=',STATEV1_IN
        WRITE(DBGUNIT,*) 'STATEV1 leaving UMAT=',STATEV(1)
        WRITE(DBGUNIT,*) 'DP=',DP,' FVAL=',FVAL,' QEQ=',QEQ
        CLOSE(DBGUNIT)
        DBG_FIRST = .FALSE.
      END IF
...
C     Frame time: 18.990257 (error: 9.7 ms from target 19.0)
      ...
      OPEN(UNIT=DBGUNIT,FILE='stage4b_sdvini_debug_trace.txt',
     1     STATUS='UNKNOWN',POSITION='APPEND')
      WRITE(DBGUNIT,*) 'SDVINI CALLED'
      WRITE(DBGUNIT,*) 'NOEL,NPT,LAYER,KSPT=',NOEL,NPT,LAYER,KSPT
      WRITE(DBGUNIT,*) 'NSTATV=',NSTATV,' NCRDS=',NCRDS
      WRITE(DBGUNIT,*) 'STATEV1 assigned=',STATEV(1)
      CLOSE(DBGUNIT)
