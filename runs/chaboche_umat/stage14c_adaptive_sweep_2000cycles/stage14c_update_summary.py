...
def write_report(stage_dir, history_rows, summary_rows):
    report_path = os.path.join(stage_dir, "STAGE14C_SWEEP_REPORT.md")
    ranked = sorted(
        summary_rows,
        key=lambda row: (
            {"accepted_clean_success": 0, "accepted_exploratory_success": 1}.get(row.get("outcome"), 2),
            float(row.get("final_statev1_error_pct") or 1.0e99),
        ),
    )
    lines = [
...
def main():
    ...
    history_path = os.path.join(args.stage_dir, "STAGE14C_SWEEP_BLOCK_HISTORY.csv")
    ...
    final = case_rows[-1]
    final_result = read_first(os.path.join(final["case_dir"], "stage14c_%s_block%02d_result.csv" % (case_id, int(final["block_id"]))))
    ...
    summary_path = os.path.join(args.stage_dir, "STAGE14C_SWEEP_CASE_SUMMARY.csv")
    ...
    write_rows(os.path.join(args.stage_dir, "STAGE14C_SWEEP_MASTER_SUMMARY.csv"), SUMMARY_FIELDS, summary_rows)
