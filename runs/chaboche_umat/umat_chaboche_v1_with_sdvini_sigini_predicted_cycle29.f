...
C ======================================================================
C SIGINI: Initialize residual stress from extracted cycle-29 data.
C ======================================================================
...
C     Predicted cycle-29 stress prepared from cycle-10 state (Stage 5A)
C     Source: stage5_predicted_cycle_jump/cycle19_predicted_stress_for_injection.csv

      IF (NTENS.GE.1) SIGMA(1) = 342.232143826245D0
      IF (NTENS.GE.2) SIGMA(2) = -2.60532336446178D-14
      IF (NTENS.GE.3) SIGMA(3) = -2.79282769750111D-14
      IF (NTENS.GE.4) SIGMA(4) = 7.780299483681D-14
      IF (NTENS.GE.5) SIGMA(5) = -2.62819331652222D-14
      IF (NTENS.GE.6) SIGMA(6) = 1.45367494564109D-14

      RETURN
      END

C ======================================================================
C SDVINI: Initialize state-dependent variables from extracted cycle-29
...
C     Predicted cycle-29 state prepared from cycle-10 state (Stage 5A)
C     Source: stage5_predicted_cycle_jump/cycle19_predicted_statev_for_injection.csv

      STATEV(1 ) = 0.20679062185814D0       ! STATEV1: accumulated viscoplastic strain
      STATEV(2 ) = -82.3499526977046D0       ! STATEV2: backstress X1
      STATEV(3 ) = 41.1749763488544D0       ! STATEV3: backstress X2
      STATEV(4 ) = 41.1749763488544D0       ! STATEV4: backstress X3
      STATEV(5 ) = 5.41768929145389D-15       ! STATEV5: near-zero shear component
      STATEV(6 ) = 2.06147328959198D-15       ! STATEV6: near-zero shear component
      STATEV(7 ) = -1.39127229941916D-15       ! STATEV7: near-zero shear component
      STATEV(8 ) = -0.00182452091636999D0       ! STATEV8: viscoplastic strain Ep1
      STATEV(9 ) = 0.000912260458183996D0       ! STATEV9: viscoplastic strain Ep2
      STATEV(10) = 0.000912260458183996D0       ! STATEV10: viscoplastic strain Ep3
      STATEV(11) = -8.59081377412822D-19       ! STATEV11: near-zero shear component
      STATEV(12) = -5.11663250425811D-19       ! STATEV12: near-zero shear component
      STATEV(13) = -6.98289811714467D-20       ! STATEV13: near-zero shear component
      STATEV(14) = 2.05725237844563D0       ! STATEV14: recomputed isotropic hardening
