...
\subsection{Prepared R3J +10 Cases}

The next conservative fixed-jump cases were prepared locally but not submitted during this update:

\begin{table}[H]
\centering
\caption{Prepared Stage 16N-R3J +10 restart-preserved jump cases.}
\begin{tabular}{lllll}
\toprule
Case & Restart & Slope pair & Material jump & Target \\
\midrule
\code{R3J3} & 250 & 100 $\rightarrow$ 250 & 250 $\rightarrow$ 260 & 500 \\
\code{R3J4} & 500 & 250 $\rightarrow$ 500 & 500 $\rightarrow$ 510 & 750 \\
\bottomrule
\end{tabular}
\end{table}

Both generated runners now use Linux repository-relative reference paths for the comparison step and search the Abaqus \code{.dat} file for the overwrite trace. The recommended progression is +10 next, +20 only if +10 passes, and adaptive $\Delta N$ restart-preserved jumping only after the fixed-jump accuracy boundary is known.
