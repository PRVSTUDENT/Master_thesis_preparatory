...
\end{quote}
It was submitted on 2026-07-01 through \code{/home/pr21vyci/bin/qsub\_abq\_guarded}, not raw \code{qsub}, as PBS job \code{1362114.mmaster02}. The pre-submission storage gate reported \code{/scratch9/pr21vyci=2.7T}. The PBS request is one CPU, 30 GB, and 24 h, with Abaqus executed as \code{cpus=1}. The scheduler snapshot after submission showed \code{job\_state=R}, \code{queue=mediumq}, \code{exec\_host=mnode028/0}, and \code{select=1:ncpus=1:mpiprocs=1:ompthreads=1:mem=30gb}.

The controller runs from scratch at:
\begin{quote}\footnotesize\ttfamily
/scratch/pr21vyci/stage16n\_r4q\_long\_adaptive\_chain\_1cpu/1362114.mmaster02
\end{quote}
and copies back only lightweight evidence. The first snapshot recorded \code{status=starting}, \code{phase=preflight}, and \code{detail=storage gate passed}. The controller log shows that the initial source datacheck completed successfully with Abaqus \code{Exit status: 0}; the initial source solve had then started. The important lightweight files are \code{R4Q\_LONG\_CHAIN\_STATUS.txt}, \code{R4Q\_LONG\_CHAIN\_BLOCK\_SUMMARY.csv}, \code{R4Q\_LONG\_CHAIN\_CONTROLLER.log}, \code{R4Q\_LONG\_CHAIN\_SUBMITTED\_JOB.txt}, and the \code{qstat\_r4q\_1362114} snapshots.
