...
The R4L redesign has now been prepared as \code{stage16n\_r4l2\_250branch\_r1b\_restart\_storage\_light\_controller}. The controller uses R1B directly as \code{oldjob=stage16n\_r1b\_restart\_ref\_250cycles}, avoids R1A completely, and does not require the broken/missing R1B \code{.odb}. The 2026-06-22 pre-submit gate found no active jobs, \code{/scratch9/pr21vyci} at about 2.7 TB, and readable R1B \code{.stt/.res/.mdl/.prt/.sim/.sta} companions; the R1B \code{.sta} completed successfully at cycle 250 with restart row \code{STEP=250, INC=58}. R4L2-1 tests the target-270 true jump and solves 271--500. R4L2-2 tests target 280 and solves 281--500 only if R4L2-1 passes. If cached jump-state files are absent, the controller records \code{blocked\_missing\_cached\_jump\_state} and stops before Abaqus.

The first R4L2 submission, job \code{1353909.mmaster02}, is not a scientific true-jump result. PBS finished with \code{Exit\_status=0} and \code{Stageout\_status=1} after 00:01:17 walltime and 00:00:17 CPU time. R1B preflight passed and cached target-270 jump state was found, but R4L2-1 failed in the Abaqus input file processor during datacheck and again during solve before a valid continuation/comparison. R4L2-2 was not run. The runner has been patched after this attempt to stop immediately on datacheck/solve failure and retain small \code{.dat} tail diagnostics before heavy cleanup.

The current operational conclusion after R4K2B and R4L source inventory is that the 250 branch is validated and ready for the R4L2 R1B storage-light true-jump controller, while the 505 branch is parked until a new exact-control gate or redesign is defined. The preserved R4E2 cycle-505 source should not be treated as a valid continuation source. R4J9/R4J10 and any broad true-jump batch remain blocked.
