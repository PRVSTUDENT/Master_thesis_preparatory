...
The R4J3 earlier-window case passes the current primary-local 5\% gate. Its controlling primary-local metric is \code{HOLE\_RING\_SDV1\_MAX}. The R4J4 later-window case fails the same gate, controlled by \code{HOLE\_RING\_MISES\_MAX}; the diagnostic \code{HOLE\_RING\_S11\_MAX\_ABS} error also rises to 13.256425\%. This means the repaired restart route remains solver-stable for true +20 skipped-cycle acceleration, but +20 is not yet a uniform safe production jump across the full benchmark. The next bracketing step should reduce the later checkpoint jump, for example to +10 or +15 from cycle 500, before claiming a safe accelerated jump size beyond the earlier 250--500 window.
