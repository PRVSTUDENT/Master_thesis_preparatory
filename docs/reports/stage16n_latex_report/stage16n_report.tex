...
The important methodological limitation is exact local reinjection. B0-1 proves that the manual \code{SDVINI}/\code{SIGINI} workflow can run and reproduce the global force-displacement response well, but it does not reproduce local strain-like state variables exactly. The cycle-100 initialization-only audit shows that this local mismatch appears immediately during manual Abaqus initialization/equilibration, even though the binary reader is exact. The new cycle-250 exact-reinjection and initialization-only failures show that the same mechanism becomes a hard equilibration gate at cycle 250.

\section{Next Work}

The completed dependency queue shows that a small jump from cycle 100 can work only in a narrow window: \code{B1D5\_100\_to\_105\_to\_250} and \code{B1D5\_EQ\_100\_to\_105\_to\_250} passed with maximum primary error 2.34986\%. The surrounding smaller and larger B1 variants remain review cases, including \code{B1D1}, \code{B1D2}, \code{B1D3}, \code{B1D4}, \code{B1Q}, \code{B1S}, and \code{B1}. The \code{EQ} fallback cases did not reduce the comparison errors. B0-2 and B0-3 remain on hold as exact-local-restart claims, and B2/B3 fixed jumps are also held because the cycle-250 reinjection/equilibration gate has not passed.

\begin{enumerate}
  \item Do not submit B2/B3 yet.
  \item Use the completed B1D4 pointwise diagnostic as evidence that the anomaly is not a simple phase mismatch or argmax-location swap.
  \item Treat the completed B1D3 result as confirmation that cycle-100 jumps are not robust under a hard local stress-maximum criterion.
  \item Use the refreshed comparison summary as the current B1 boundary result: only \code{B1D5} and \code{B1D5\_EQ} pass the 5\% primary-error gate.
  \item Do not rerun the \code{EQ} variants just to change the equilibration initial increment; those reruns reproduced the same comparison errors.
  \item Diagnose the cycle-250 reinjection/equilibration failure before rerunning \code{B2SAFE}.
  \item Treat the failed \code{B0\_AUDIT\_0250\_INITIALIZATION\_ONLY} job as evidence that the cycle-250 failure appears immediately during manual initialization/equilibration.
  \item Avoid or re-score cycle-100 jumps using robust percentile-based local stress metrics if hard local maxima remain too brittle for acceptance.
  \item Use the submitted \code{SDVINI}-only and \code{SIGINI}-only audits to isolate whether stress, STATEV, or coupled field injection controls the cycle-250/500 equilibration barrier.
