...
The R4E errors are identical to the preceding R4J7/R4J8 comparison summaries, even though R4E was meant to replace the extrapolated jump state with an exact target state. This is a decisive audit signal. The current evidence does not support continuing directly to R4J9/R4J10. Before any additional extrapolated refinement is submitted, the exact-state extraction, state-file handoff, overwrite activation/timing, and comparison mapping must be audited.

The current thesis conclusion is therefore intentionally cautious: the R4J true-skip refinement shows branch-dependent local error, but the exact-target controls did not pass. The safe-jump bracket cannot be finalised until the R4E failure mechanism is understood. Heavy Abaqus files remain in scratch; only lightweight evidence, comparison CSVs, status notes, selected-cycle CSVs, \code{.sta} files, PBS stdout, and small logs were copied into the repository.
