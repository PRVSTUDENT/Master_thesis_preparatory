...
R4I-R5 and R4I-R6 were submitted as the next two-job deck-clone confirmation wave on 2026-06-21 after the guarded storage gate passed. The gate reported \code{/home} 17T total, 13T used, 3.4T free; \code{/scratch} 110T total, 82T used, 28T free; \code{/home/pr21vyci} 66G; project 56G; and no large Abaqus files in the home project. Both jobs were submitted through the guarded R4I-R dispatcher, not raw unguarded \code{qsub}. R4I-R5 is \code{1350742.mmaster02}, deck-clone source 250--271 with restart 270--500, running on \code{mnode043}. R4I-R6 is \code{1350743.mmaster02}, deck-clone source 500--506 with restart 505--750, running on \code{mnode051}. Both request \code{select=1:ncpus=16:mpiprocs=1:ompthreads=16:mem=90gb}, \code{walltime=24:00:00}, write PBS stdout under \code{/scratch/pr21vyci/stage16n\_r4ir\_pbs/}, and execute in scratch under \code{/scratch/pr21vyci/stage16n\_r4ir/}. The scratch directories already contain staged case files, copied reference CSVs, and active source-solve Abaqus outputs, confirming that the jobs reached real execution.

If R4I-R5 passes, deck-clone/truncate fixes the 270--500 branch. If R4I-R6 passes, deck-clone/truncate fixes the 505--750 branch. If both pass, future short-source generation should be replaced with deck-clone/truncate source generation, followed by deck-clone exact controls R4K1 and R4K2 before any true extrapolated jump cases are unblocked. If either fails, that branch should be inspected for restart-cycle read/write or reference-mapping issues.

After R4I-R5/R4I-R6 are classified, later R4K or resource-calibration jobs should add explicit phase timing around the source solve, continuation solve, ODB extraction, copy-back, and comparison. The timing wrapper should print a phase label, start/end timestamps, exit code, and \code{/usr/bin/time -v} resource report for each major command. If R4I-R5/R4I-R6 also average roughly four active cores over the full workflow, the next resource study should compare one representative case at 8 cores versus 16 cores, holding the numerical setup fixed and comparing walltime, CPU efficiency, and numerical results before changing the default Stage 16N production allocation.

The uploaded lightweight evidence for R4I-R3 and R4I-R4 is in \code{runs/chaboche\_umat/stage16\_abaqus\_inhomogeneous\_cycle\_jump\_benchmark/stage16n\_restart\_control/r4ir\_restart\_source\_buffer\_recovery/}. Each second-wave case directory contains the case status markdown, comparison summary/details CSVs, cycle metrics CSVs, selected-cycle loop/local-state CSVs, copied reference CSVs, forced-added PBS stdout, and \code{qstat\_<jobid>\_finished\_full.txt}.
