...
The UMAT overwrite trace confirms that the state overwrite fired at the intended restart continuation hook: \code{KSTEP=251} for R3J5 and \code{KSTEP=501} for R3J6, with 9 diagnostic trace lines in each case. The Stage 16N-R conclusion is therefore stronger again: the restart-preserved UMAT cycle-jump workflow has now passed nonzero +5, +10, and +20 jumps. The remaining scientific question is the jump-size boundary at which endpoint and local metric error becomes nonzero.

Lightweight R3J3/R3J4 evidence was copied into the repository. Heavy Abaqus result and restart files, including \code{.stt}, \code{.odb}, \code{.res}, \code{.sim}, \code{.mdl}, \code{.prt}, large \code{.msg}, and large \code{.dat} files, are intentionally excluded from GitHub and are being offloaded from \code{/home} to \code{/scratch}.
