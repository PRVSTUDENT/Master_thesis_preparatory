...
\section{Stage 16N-B: Exact Reinjection Verification}
...
\section{B0-1 Exact Cycle-100 to Cycle-250 Reinjection}
...
Route & exact cycle 100 state $\rightarrow$ cycle 250 \\
...
\begin{enumerate}
  \item Add a true Abaqus restart comparison from cycle 100 to cycle 250, if restart data are available.
  \item Investigate whether a displacement/strain-field initialization or predefined field strategy can reduce the local \code{SDV8} mismatch.
  \item If manual \code{SDVINI}/\code{SIGINI} is retained as the practical state-jump method, document that it is not a mathematically exact restart and use local equilibration error as part of the cycle-jump error budget.
  \item Only after this decision, run B0-2 and B0-3 or begin conservative fixed jump tests.
\end{enumerate}
