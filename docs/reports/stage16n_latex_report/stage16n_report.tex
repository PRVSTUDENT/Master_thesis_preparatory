...
To avoid repeating the HPC \code{/home} storage issue, the R4J pair is submitted through the scratch watchdog \code{watch\_cleanup\_and\_submit\_stage16n\_scratch\_jobs.sh}. This script waits for ongoing offload work to finish, verifies \code{/home} and \code{/scratch} usage, checks that no large Abaqus heavy files remain in the \code{/home} project tree, stages only the required case folders, helper scripts, reference CSV files, and restart-source symlinks to \code{/scratch/pr21vyci/stage16n\_scratch\_runs\_r4j\_v2/Abaqus\_trial}, submits PBS wrappers from the \code{/home} case folders, runs Abaqus in the scratch case directories, and copies back only lightweight evidence. The final submission required the same queue directive used by accepted Stage 16N jobs, \code{\#PBS -q entry\_teachingq}. With that fix, R4J1 and R4J2 were submitted on Sat Jun 13 06:01 CEST 2026 as jobs \code{1344946.mmaster02} and \code{1344947.mmaster02}; both were running at the status check on \code{mnode100} and \code{mnode101}, respectively.

Lightweight R3J5/R3J6 evidence was copied into the repository. Heavy Abaqus result and restart files, including \code{.stt}, \code{.odb}, \code{.res}, \code{.sim}, \code{.mdl}, \code{.prt}, large \code{.msg}, and large \code{.dat} files, are intentionally excluded from GitHub and are being offloaded from \code{/home} to \code{/scratch}.
