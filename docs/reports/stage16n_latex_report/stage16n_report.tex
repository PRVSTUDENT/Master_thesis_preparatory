...
\subsection{R4J Branch-Refinement Scratch Submission}

The branch-refinement pair was submitted from the HPC Git clone after switching to the HPC-first workflow. The first two submission attempts, jobs \code{1345007}/\code{1345008} and \code{1345009}/\code{1345010}, exited before Abaqus because the scratch-staged cases did not link the native restart source \code{stage16n\_r1a\_restart\_ref\_500cycles.odb}. These are setup failures and are not interpreted as R4J accuracy results.

The scratch workflow was then corrected in two small GitHub commits. Commit \code{df7facc} made the generated restart linker fall back to the canonical HPC restart-reference directory when the heavy restart files are intentionally not staged into scratch. Commit \code{ee5ddbe} made the scratch PBS wrapper run \code{link\_restart\_sources.sh} whenever the script is present, rather than only when the file has executable mode. After these fixes, R4J5 and R4J6 were relaunched as jobs \code{1345011.mmaster02} and \code{1345012.mmaster02}.

\subsection{R4J Branch-Specific Refinement Result}

The branch-specific R4J5/R4J6 pair was checked from PBS history and scratch evidence on 2026-06-15. Both production solver jobs completed successfully with \code{Exit\_status=0}; the completed production jobs were \code{1345011.mmaster02} and \code{1345012.mmaster02}.
...
The updated branch-specific bracket is: for the cycle-250 branch, +20 passed, +35 is review/fail against the 5\% gate, and +50 failed; for the cycle-500 branch, +10, +20, and +50 all exceeded the local gate. The next useful tests are therefore below +35 for the 250 branch, specifically +30, and below +10 for the 500 branch, specifically +5.
