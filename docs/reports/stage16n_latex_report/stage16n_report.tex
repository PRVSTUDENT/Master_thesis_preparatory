...
R4Q is the active long-chain Abaqus test of the validated 21-cycle adaptive recommendation. It is not a new boundary-widening study and should not be duplicated while the submitted controller is active. The controller applies the already accepted safe jump pattern over sequential 250-cycle blocks:
\begin{quote}
...
The completed R4Q result is partial feasibility evidence. Block 1 completed source250 \(\rightarrow\) target271, solved 272--500, and extracted a new cycle-500 source state; no comparison reference was available for that endpoint, so it is not an accuracy-validation claim. Block 2 stopped before solving. The controller produced the source500 \(\rightarrow\) target521 extrapolated state, but \code{stage16n\_make\_r4q\_restart\_deck.py} received an empty \code{--old-inc}, so the block-2 input deck was not generated and Abaqus datacheck reported the missing \code{stage16n\_r4q\_block02\_500\_to\_521\_solve\_522\_to\_750.inp}. This is a controller/deck-generation stop, not a scientific rejection of the 500 \(\rightarrow\) 521 \(\rightarrow\) 750 adaptive jump. The next step is a cheap no-solver preflight that proves the block-2 restart step/increment and generated input deck before any further Abaqus solve is submitted.
