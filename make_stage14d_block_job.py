...
def block_dir(stage14d, strategy, block_index, base_cycle, target_cycle, continue_to_cycle):
    return os.path.join(
        stage14d,
...
def row_delta(row, name):
    if name.startswith("STATEV"):
        return row.get("Delta_%s" % name)
    if name in STRESS_COMPONENTS:
        return row.get("Delta_%s" % name)
    return None
...
def row_value(row, name):
    if name.startswith("STATEV"):
        return row.get("%s_end" % name)
    return row.get(name)


def control_variable_names():
    if DELTA_N_CONTROL_VARIABLES == "STATEV1_only":
        return ["STATEV1"]
    if DELTA_N_CONTROL_VARIABLES == "STATEV1_plus_S11":
        return ["STATEV1", "S11"]
    if DELTA_N_CONTROL_VARIABLES in ("STATEV1_plus_S11_RF1", "STATEV1_plus_S11_plus_RF1"):
        return ["STATEV1", "S11"]
    if DELTA_N_CONTROL_VARIABLES == "active_STATEVs":
        return ["STATEV%d" % i for i in sorted(ACTIVE_STATEV)]
    if DELTA_N_CONTROL_VARIABLES == "active_STATEVs_plus_S11_RF1":
        return ["STATEV%d" % i for i in sorted(ACTIVE_STATEV)] + ["S11"]
    return ["STATEV1"]
...
def choose_adaptive_delta_n(source_rows, base_cycle, mean_start, mean_end):
    ...
    if max_allowed <= 0:
        ...
        return tail_delta, 0.0, 0.0, 0.0, tail_delta
    search_max = min(DN_MAX, max_allowed)
    controls = []
    for name in control_variable_names():
        deltas = [row_delta(row, name) for row in window if row_delta(row, name) is not None]
        if len(deltas) < 5:
            continue
        m = robust_mean(deltas)
        c = robust_mean([deltas[i] - deltas[i - 1] for i in range(1, len(deltas))]) or 0.0
        p_base = row_value(base, name)
        if p_base is None:
            continue
        controls.append((name, p_base, m, c))
    if not controls:
        raise RuntimeError("Need at least one usable adaptive control variable for %s" % DELTA_N_CONTROL_VARIABLES)

    best = 1
    best_err = None
    for dn in range(1, search_max + 1):
        err = 0.0
        for _name, p_base, m, c in controls:
            denom = max(abs(p_base + m * dn), EPS)
            err = max(err, abs(0.5 * c * dn * dn) / denom)
        if err <= LOCAL_TOL:
            best = dn
            best_err = err
    ...
    for _name, p_base, m, c in controls:
        ...
        err = max(err, abs(0.5 * c * safe * safe) / denom)
    statev1 = [item for item in controls if item[0] == "STATEV1"]
    if statev1:
        _name, _base, m_report, c_report = statev1[0]
    else:
        _name, _base, m_report, c_report = controls[0]
    return safe, m_report, c_report, err, best
...
def compute_prediction(source_rows, source_csv, ref_rows, ref_csv, base_cycle, target_cycle, continue_to_cycle, mean_start, mean_end):
    ...
    for i in range(1, NSTATEV + 1):
        ...
        if PREDICTION_ORDER == "second_order":
            increments = [row["Delta_STATEV%d" % i] for row in window if row.get("Delta_STATEV%d" % i) is not None]
            curvature = robust_mean([increments[j] - increments[j - 1] for j in range(1, len(increments))]) or 0.0
            pred_statev[i] = base["STATEV%d_end" % i] + delta_n * statev_mean[i] + 0.5 * curvature * delta_n * delta_n
        else:
    ...
    for name in STRESS_COMPONENTS:
        ...
        if PREDICTION_ORDER == "second_order":
            increments = [row["Delta_%s" % name] for row in window if row.get("Delta_%s" % name) is not None]
            curvature = robust_mean([increments[j] - increments[j - 1] for j in range(1, len(increments))]) or 0.0
            pred_stress[name] = base[name] + delta_n * stress_mean[name] + 0.5 * curvature * delta_n * delta_n
        else:
...
def write_deck(path, job, strategy, block_index, target_cycle, continue_to_cycle):
    ...
    if continuation_cycles < 1 or continuation_cycles > RECOVERY_WINDOW:
        raise RuntimeError("Stage 14D blocks must continue 1-%d cycles; got %d" % (RECOVERY_WINDOW, continuation_cycles))
    lines = [
...
def write_prep_report(path, strategy, block_index, base_cycle, target_cycle, continue_to_cycle, base_source, source_csv, mean_start, mean_end, pred_statev, pred_stress, exact, ref_continue):
    ...
    lines = [
        "# Stage 14D %s Block %02d Prep Report" % (strategy, block_index),
        "",
        "## Route",
        "",
...
def create_block(args):
    ...
    stage14d = os.path.join(repo, "runs", "chaboche_umat", "stage14d_24h_controller_sweep_2000cycles")
    ...
    case_dir = block_dir(stage14d, STRATEGY_LABEL, args.block_index, args.base_cycle, target_cycle, continue_to_cycle)
    if not os.path.exists(case_dir):
        os.makedirs(case_dir)

    job = "chaboche_stage14d_%s_block%02d_target%d_to%d" % (STRATEGY_LABEL, args.block_index, target_cycle, continue_to_cycle)
    previous_csv = args.previous_route_csv
    route_csv_name = "route_history_until_cycle%d.csv" % continue_to_cycle
    result_csv_name = "stage14d_%s_block%02d_result.csv" % (STRATEGY_LABEL, args.block_index)
    prep_report_name = "STAGE14D_%s_BLOCK%02d_PREP_REPORT.md" % (STRATEGY_LABEL.upper(), args.block_index)
    result_report_name = "STAGE14D_%s_BLOCK%02d_RESULT_REPORT.md" % (STRATEGY_LABEL.upper(), args.block_index)
    ...
    paths = {
        ...
        "runner": os.path.join(case_dir, "run_stage14d_%s_block%02d_target%d_to%d.bat" % (STRATEGY_LABEL, args.block_index, target_cycle, continue_to_cycle)),
        ...
        "metadata": os.path.join(case_dir, "stage14d_%s_block%02d_metadata.csv" % (STRATEGY_LABEL, args.block_index)),
    }
...
def main():
    ...
    global STRATEGY_LABEL, PREDICTION_ORDER, DELTA_N_CONTROL_VARIABLES, INJECTION_MODE, ROLLBACK_ENABLED
    ...
    parser.add_argument(
        ...
        choices=["STATEV1_only", "STATEV1_plus_S11", "STATEV1_plus_S11_RF1", "STATEV1_plus_S11_plus_RF1", "active_STATEVs", "active_STATEVs_plus_S11_RF1"],
    ...
    parser.add_argument("--rollback-enabled", choices=["true", "false"], default=ROLLBACK_ENABLED)
